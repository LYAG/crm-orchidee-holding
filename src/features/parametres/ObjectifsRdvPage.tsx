...
import { objectifRdvService, utilisateurService } from '@/services';
import type { ObjectifMois, ObjectifRdvAnnee, Utilisateur } from '@/types';
...
export function ObjectifsRdvPage() {
  const { user } = useAuth();
  ...
  const [donnees, setDonnees] = useState<ObjectifRdvAnnee | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    utilisateurService
      .getDeleguesByManager(user.id)
      .then((liste) => {
        setDelegues(liste);
        setDelegueId((prev) => prev ?? liste[0]?.id);
      })
      .catch(() => message.error('Erreur lors du chargement de votre équipe.'));
  ...
  async function enregistrerDefaut() {
    if (defautValeur == null || !delegueId) return;
    setDefautSauvegarde(true);
    try {
      const maj = await objectifRdvService.definir(delegueId, annee, null, defautValeur);
      setDonnees(maj);
      setDefautEnEdition(false);
  ...
  async function enregistrerMois(mois: number) {
    if (moisValeur == null || !delegueId) return;
    setMoisSauvegarde(true);
    try {
      const maj = await objectifRdvService.definir(delegueId, annee, mois, moisValeur);
      setDonnees(maj);
      setMoisEnEdition(null);
  ...
  async function reinitialiserMois(mois: number) {
    if (!delegueId) return;
    try {
      const maj = await objectifRdvService.reinitialiserMois(delegueId, annee, mois);
      setDonnees(maj);
  ...
  return (
    <PageContainer
      title="Objectifs de RDV"
      subTitle="Par délégué et par mois"
      tags={
        <Tag
          icon={<SettingOutlined />}
          style={{ background: '#E3F2FD', color: '#1565C0', border: 'none', fontWeight: 600, borderRadius: 6 }}
        >
          Manager
        </Tag>
      }
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12 }}>
                  Nombre de RDV que le délégué doit réaliser
                </Text>
              </div>
            </div>
            <Space>
